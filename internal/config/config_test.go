...
func TestLoadChannelsOnlyFileNotFound(t *testing.T) {
	withIsolatedEnv(t, nil, func() {
		withProjectFileContents(t, filepath.Join("configs", "channels.yml"), nil, func() {
			cfg := defaultConfig()
			entries, err := LoadChannelsOnly(cfg, ProjectFile(filepath.Join("configs", "channels.yml")))
			if err != nil {
				t.Fatalf("LoadChannelsOnly: %v", err)
			}
			if entries != nil && len(entries) > 0 {
				t.Fatalf("expected nil/empty for missing file, got %d entries", len(entries))
			}
		})
	})
}

func TestLoadChannelsOnlyValidYAML(t *testing.T) {
	withIsolatedEnv(t, nil, func() {
		content := "" +
			"channels:\n" +
			"  wecom:\n" +
			...
			"      - assistant\n" +
			"    gateway:\n" +
			"      url: ws://127.0.0.1:11971/ws/agent?channel=feishu\n"
		withProjectFileContents(t, filepath.Join("configs", "channels.yml"), &content, func() {
			cfg := defaultConfig()
			cfg.GatewayWS.HandshakeTimeoutMs = 10000
			cfg.GatewayWS.ReconnectMinMs = 1000
			cfg.GatewayWS.ReconnectMaxMs = 30000
			entries, err := LoadChannelsOnly(cfg, ProjectFile(filepath.Join("configs", "channels.yml")))
			if err != nil {
				t.Fatalf("LoadChannelsOnly: %v", err)
			}
			...
			byID := map[string]GatewayEntry{}
			for _, e := range entries {
				byID[e.ID] = e
			}
			if byID["wecom"].Channel != "wecom" {
				t.Fatalf("wecom channel = %q, want wecom", byID["wecom"].Channel)
			}
			if byID["wecom"].URL != "ws://127.0.0.1:11970/ws/agent?channel=wecom" {
				t.Fatalf("wecom URL = %q", byID["wecom"].URL)
			}
			if byID["wecom"].JwtToken != "jwt-token-123" {
				t.Fatalf("wecom token = %q", byID["wecom"].JwtToken)
			}
			if byID["feishu"].Channel != "feishu" {
				t.Fatalf("feishu channel = %q", byID["feishu"].Channel)
			}
			// Default reconnect values from cfg
			if byID["wecom"].ReconnectMinMs != 1000 {
				t.Fatalf("wecom reconnect min = %d", byID["wecom"].ReconnectMinMs)
			}
		})
	})
}

func TestLoadChannelsOnlyInvalidYAML(t *testing.T) {
	withIsolatedEnv(t, nil, func() {
		content := "channels:\n  invalid: ["
		withProjectFileContents(t, filepath.Join("configs", "channels.yml"), &content, func() {
			cfg := defaultConfig()
			_, err := LoadChannelsOnly(cfg, ProjectFile(filepath.Join("configs", "channels.yml")))
			if err == nil {
...
func TestLoadChannelsOnlyEmptyFile(t *testing.T) {
	withIsolatedEnv(t, nil, func() {
		content := "{}\n"
		withProjectFileContents(t, filepath.Join("configs", "channels.yml"), &content, func() {
			cfg := defaultConfig()
			...
			if entries != nil && len(entries) > 0 {
				t.Fatalf("expected nil/empty for empty file, got %d entries", len(entries))
			}
		})
	})
}
