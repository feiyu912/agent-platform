...
func TestSubmitReturnsAlreadyResolvedWhenAwaitingWasConsumed(t *testing.T) {
	fixture := newTestFixture(t)

	const runID = "resolved-run"
	const awaitingID = "resolved-awaiting"
	_, runControl, _ := fixture.runs.Register(context.Background(), contracts.QuerySession{
		RunID:    runID,
	...
	firstAck := fixture.runs.Submit(api.SubmitRequest{
		RunID:      runID,
		AwaitingID: awaitingID,
		Params:     mustEncodeSubmitParams(t, []map[string]any{{"id": "q1", "answer": "ok"}}),
	})
	if !firstAck.Accepted || firstAck.Status != "accepted" {
		t.Fatalf("expected first submit accepted, got %#v", firstAck)
	}

	req := httptest.NewRequest(http.MethodPost, "/api/submit", bytes.NewBufferString(`{"runId":"`+runID+`","awaitingId":"`+awaitingID+`","params":[{"id":"q1","answer":"ok"}]}`))
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	fixture.server.ServeHTTP(rec, req)
	if rec.Code != http.StatusOK {
	...
	if submitResp.Data.Accepted || submitResp.Data.Status != "already_resolved" {
		t.Fatalf("expected already_resolved submit response, got %#v", submitResp.Data)
	}
}
