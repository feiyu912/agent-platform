...
- [internal/server/memory_learning.go](internal/server/memory_learning.go)
- [internal/memory/store.go](/store.go)
- [internal/memory/sqlite_store.go](/sqlite_store.go)
- [internal/memory/context_builder.go](/context_builder.go)
- [internal/memory/lifecycle.go](/lifecycle.go)
- [internal/memory/extractor.go](/extractor.go)
- [internal/memory/feedback.go](/feedback.go)
- [internal/chat/search.go](internal/chat/search.go)
- [internal/tools/tool_memory.go](internal/tools/tool_memory.go)
...
生产实现是 [internal/memory/sqlite_store.go](/sqlite_store.go) 里的 `SQLiteStore`。
...
在 [internal/memory/types.go](/types.go) 里，最核心的类型区分只有两个：
...
scope 在 [internal/memory/types.go](/types.go) 里定义为：
...
入口在 [internal/server/handler_query.go](internal/server/handler_query.go)。收到 query 后，如果 memory 已配置，会调用：

```go
s.deps.Memory.BuildContextBundle(memory.ContextRequest{...})
```

真正的分层选择逻辑在 [internal/memory/context_builder.go](/context_builder.go)。
...
再由 [internal/llm/prompt_builder.go](internal/llm/prompt_builder.go) 拼进最终系统 prompt。
...
[internal/memory/context_builder.go](/context_builder.go) 里的 `computeEffectiveImportance(...)` 不只看 `Importance`，还看：
...
[internal/server/handler_memory.go](internal/server/handler_memory.go) 里的 `handleRemember` 会从 chat detail 提取摘要，然后 `Memory.Remember(...)` 写入。
...
[internal/memory/extractor.go](/extractor.go) 当前实现相对保守：
...
整理逻辑在 [internal/memory/lifecycle.go](/lifecycle.go)。
...
晋升后在 [internal/memory/sqlite_store.go](/sqlite_store.go) 里会：
...
[internal/memory/feedback.go](/feedback.go) 负责计算反馈信号。
...
`_session_search_` 工具定义在 [internal/resources/tools/_session_search_.yml](internal/resources/tools/_session_search_.yml)，实现位于 [internal/tools/tool_session_search.go](internal/tools/tool_session_search.go) 和 [internal/chat/search.go](internal/chat/search.go)。
...
在 [internal/app/app.go](internal/app/app.go) 中，memory store 旁边还初始化了 `skill-candidates` 存储。
...
[internal/memory/safety.go](/safety.go) 会拦截几类风险内容：
...
- `journal`
  - [internal/memory/journal.go](/journal.go)
  - 每次写入都会按日期追加 markdown 日志
- `snapshot / exports`
  - [internal/memory/snapshot_renderer.go](/snapshot_renderer.go)
