...
CODER 可通过 `runtimeConfig.coderBackend: acp` 改由全局 `CODER_ACP_BASE_URL` 指向的 ACP bridge 执行。此时 agent 仍是 `mode: CODER`，但执行链路会转发到 `proxy-acp-codex`：
...
- `runtimeConfig.coderBackend` 仅对 `mode: CODER` 生效，可取 `native` 或 `acp`，默认 `native`
