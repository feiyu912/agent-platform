...
`configs/container-hub.example.yml` 只默认展开 `base-url`；其它字段已有代码默认值，模板中以注释形式保留。`auth-token` 不在模板中展示，但 `CONTAINER_HUB_AUTH_TOKEN` / `auth-token` 仍可作为客户端 Bearer token 对接 `agent-container-hub` 的 `AUTH_TOKEN`。
...
- 配置目录固定为项目根下 `configs/`
- 旧拆分文件 `access-policy.yml`、`bash.yml`、`file-tools.yml`、`coder-prompts.yml`、`memory-prompts.yml`、`vision-recognize.yml` 仍兼容读取一版，但会被新合并文件覆盖；新部署不再生成这些旧文件模板
