...
| `BUDGET_HITL_TIMEOUT_MS` | `0` | `Advanced / operator` | 全局 HITL/awaiting 等待超时；未设置时内置 fallback 为 `600000` |
| `BUDGET_HITL_QUESTION_TIMEOUT_MS` | `0` | `Advanced / operator` | `question` 等待超时；大于 0 时优先于 `BUDGET_HITL_TIMEOUT_MS` |
| `BUDGET_HITL_APPROVAL_TIMEOUT_MS` | `0` | `Advanced / operator` | `approval` 等待超时；具体审批项 `timeoutMs` 优先于该值 |
| `BUDGET_HITL_FORM_TIMEOUT_MS` | `0` | `Advanced / operator` | `form` 等待超时；具体 form/tool 或 bash-hook `timeoutMs` 优先于该值 |
| `BUDGET_HITL_PLAN_TIMEOUT_MS` | `0` | `Advanced / operator` | `plan` 等待超时；大于 0 时优先于 `BUDGET_HITL_TIMEOUT_MS` |
...
Bash 工具从 `configs/host-tools.yml` 的 `bash` 节读取命令白名单、超时和可选 host shell 覆盖；目录权限统一从同一文件的 `access-policy` 节读取。旧 `configs/bash.yml` 不再读取。参考模板是 `configs/host-tools.example.yml`。
...
`bash HITL` 不再有单独的 `.env` 开关。是否拦截 bash 命令完全由当前 agent 挂载的 skills 决定：规则来自 skill 目录下的 `.bash-hooks/`，未挂载任何 bash hooks 时不会触发拦截。旧字段 `bash.hitl-default-timeout-ms` 已删除；请迁移到 `budget.hitl.timeoutMs`，具体 approval/form 可继续在 bash-hook subcommand 或 form/tool 定义上配置 `timeoutMs`。
...
`configs/host-tools.yml` 的 `access-policy` 是 host 工具目录权限的最终事实源，`bash` 与 FileTools 共用。旧 `configs/access-policy.yml` 不再读取。请求可传 `accessLevel`，合法值为 `default`、`auto_approve`、`full_access`，缺省为 `default`。
...
`file_read` / `file_glob` / `file_grep` / `file_write` / `file_edit` 是 Claude Code 风格的窄文件工具，行为配置从 `configs/host-tools.yml` 的 `file-tools` 节读取，目录权限从同一文件的 `access-policy` 节读取；参考模板是 `configs/host-tools.example.yml`。旧 `configs/file-tools.yml` 不再读取。它们用于替代 heredoc、重定向等容易触发 bash 安全拦截的文件读写场景，不改变 bash 安全策略。
...
默认 `accessLevel=default` 下，workspace、chat、agent、当前 agent skills 读免审，workspace 和 chat 写免审；`@chat` 解析为当前 `CHATS_DIR/<chatId>`，不依赖 `@workspace == @chat` 才可写。除此之外 read/glob/grep/write/edit 都按 `host-tools.access-policy` 触发审批或自动处理。旧字段 `allowed-read-paths` / `allowed-write-paths` 已移除，仍出现在 `configs/host-tools.yml` 的 `file-tools` 节或旧 `configs/file-tools.yml` 会导致启动失败。
