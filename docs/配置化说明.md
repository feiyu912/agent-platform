...
| `AGENT_DEFAULT_BUDGET_TIMEOUT` | `300` | `Advanced / operator` | run 默认预算超时，单位秒 |
...
| `BUDGET_HITL_APPROVAL_TIMEOUT` | `0` | `Advanced / operator` | `approval` 等待超时，单位秒；具体审批项 `timeoutMs` 优先于该值 |
| `BUDGET_HITL_FORM_TIMEOUT` | `0` | `Advanced / operator` | `form` 等待超时，单位秒；具体 form/tool 或 bash-hook `timeoutMs` 优先于该值 |
...
注意：配置项中的 `BUDGET_HITL_*_TIMEOUT=0` 表示未配置；事件协议中的 `awaiting.ask.timeout=0` 表示该等待项无限等待、不自动超时。当前 awaiting/deferred/frontend wire 仍使用毫秒，配置中的秒会在运行时转换。
...
`bash HITL` 不再有单独的 `.env` 开关。是否拦截 bash 命令完全由当前 agent 挂载的 skills 决定：规则来自 skill 目录下的 `.bash-hooks/`，未挂载任何 bash hooks 时不会触发拦截。旧字段 `bash.hitl-default-timeout-ms` 已删除；请迁移到 `budget.hitl.timeout`，具体 approval/form 可继续在 bash-hook subcommand 或 form/tool 定义上配置 `timeoutMs`。
