...
approval action 支持 `allow`、`hitl`、`auto`、`block`。`auto_approve` 默认继承 `default` 边界，越界读、复杂 bash 与不透明命令自动批准，越界写仍触发 HITL；自动批准会以 `decision: auto_approved` 写入当前 `chatId.jsonl` 的 step `approval`。`full_access` 默认把 host path 读写根设为 `/`，不产生越界审批记录，但仍保留 bashsec hard block。
