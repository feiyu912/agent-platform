...
`.env.example` 现在只保留推荐给普通部署者的最终用户配置入口。本页则保留完整的高级、排障和低频配置说明；host 工具配置已统一迁移到 `configs/host-tools.yml`。
...
`runtimeConfig.extraMounts` 会真实影响 Container Hub session payload：
...
`configs/runtime.example.yml` 的 `container-hub` 节只默认展开 `base-url`；其它字段已有代码默认值，模板中以注释形式保留。`auth-token` 不在模板中展示，但 `CONTAINER_HUB_AUTH_TOKEN` / `container-hub.auth-token` 仍可作为客户端 Bearer token 对接 `agent-container-hub` 的 `AUTH_TOKEN`。
...
| `AGENT_DEFAULT_BUDGET_MODEL_MAX_CALLS` | `100` | `Advanced / operator` | legacy：模型最大调用次数，未配置 `AGENT_DEFAULT_BUDGET_MAX_STEPS` 时作为 fallback |
...
| `AGENT_DEFAULT_BUDGET_TOOL_MAX_CALLS` | `60` | `Advanced / operator` | 工具最大调用次数 |
...
| `AGENT_DEFAULT_REACT_MAX_STEPS` | `60` | `Advanced / operator` | legacy：React 模式最大步数 |
| `AGENT_DEFAULT_PLAN_EXECUTE_MAX_STEPS` | `60` | `Advanced / operator` | legacy：Plan Execute 默认最大步数 |
| `AGENT_DEFAULT_PLAN_EXECUTE_MAX_WORK_ROUNDS_PER_TASK` | `6` | `Advanced / operator` | legacy：单任务默认最大 work rounds |
...
Bash 工具从 `configs/host-tools.yml` 的 `bash` 节读取命令白名单、超时和可选 host shell 覆盖；目录权限统一从同一文件的 `access-policy` 节读取。旧 `configs/bash.yml` 仍兼容读取，但会被 `configs/host-tools.yml` 覆盖。参考模板是 `configs/host-tools.example.yml`。

| YAML 字段 | 默认值 | 标签 | 说明 |
|---|---|---|---|
| `allowed-commands` | `ls,pwd,cat,head,tail,top,free,df,git,rg,find,...` | `Advanced / operator` | 允许执行的命令白名单 |
| `shell-timeout-ms` | `10000` | `Advanced / operator` | 单条 shell 命令默认超时时间，毫秒 |
...
旧字段 `allowed-paths`、`path-checked-commands`、`path-check-bypass-commands` 已迁移到 `configs/host-tools.yml` 的 `access-policy`；启动时仍出现在 `bash` 节或旧 `bash.yml` 中会直接报错。

### Access Policy 目录权限

`configs/host-tools.yml` 的 `access-policy` 是 host 工具目录权限的最终事实源，`bash` 与 FileTools 共用。旧 `configs/access-policy.yml` 仍兼容读取，但会被 `configs/host-tools.yml` 覆盖。请求可传 `accessLevel`，合法值为 `default`、`auto_approve`、`full_access`，缺省为 `default`。

| YAML 字段 | 默认值 | 标签 | 说明 |
|---|---|---|---|
| `working-directory` | `@workspace` | `Advanced / operator` | 目录策略解析相对路径时的默认基准 |
| `levels.default.read-roots` | `@workspace,@agent,@skills` | `Advanced / operator` | 默认免审读取根 |
| `levels.default.write-roots` | `@workspace` | `Advanced / operator` | 默认免审写入根 |
| `levels.default.readonly-roots` | `@agent,@skills,@skills-market` | `Advanced / operator` | 即使被写根覆盖也保持只读的根 |
| `levels.default.approvals.read-outside-roots` | `hitl` | `Advanced / operator` | `file_read` / `file_grep` 或 bash 读取越界路径的处理 |
...
`file_read` / `file_grep` / `file_write` / `file_edit` 是 Claude Code 风格的窄文件工具，行为配置从 `configs/host-tools.yml` 的 `file-tools` 节读取，目录权限从同一文件的 `access-policy` 节读取；参考模板是 `configs/host-tools.example.yml`。旧 `configs/file-tools.yml` 仍兼容读取，但会被 `configs/host-tools.yml` 覆盖。它们用于替代 heredoc、重定向等容易触发 bash 安全拦截的文件读写场景，不改变 bash 安全策略。

`file_write` / `file_edit` 成功结果会包含 `lineStats`，字段为 `addedLines`、`deletedLines`、`editedLines`。统计基于写入前后完整文件内容的行级 diff；新建文件等价于从空内容写入新内容。

| YAML 字段 | 默认值 | 标签 | 说明 |
|---|---|---|---|
| `working-directory` | `.` | `Advanced / operator` | `file_read` / `file_grep` / `file_write` / `file_edit` 解析相对路径时使用的工作目录；路径权限仍以 `host-tools.access-policy` 为准 |
...
默认 `accessLevel=default` 下，workspace、agent、当前 agent skills 读免审，workspace 写免审；除此之外 read/grep/write/edit 都按 `host-tools.access-policy` 触发审批或自动处理。旧字段 `allowed-read-paths` / `allowed-write-paths` 已移除，仍出现在 `configs/host-tools.yml` 的 `file-tools` 节或旧 `configs/file-tools.yml` 会导致启动失败。
...
配置从 `configs/ai-tools.yml` 的 `vision-recognize` 节读取，参考模板是 `configs/ai-tools.example.yml`。旧 `configs/vision-recognize.yml` 仍兼容读取，但会被 `configs/ai-tools.yml` 覆盖。
...
| `profiles.<name>.timeout-ms` | `60000` | `Advanced / operator` | 单次识别请求超时 |
...
图片来源支持当前 chat 附件 `reference_name`，以及受 `host-tools.access-policy` 读权限约束的本地 `file_path`。v1 不支持远程 URL。
...
`STREAM_INCLUDE_TOOL_PAYLOAD_EVENTS` 和 `DEBUG_EVENTS_ENABLED` 会出现在 `.env.example` 中，方便最终用户发现和按需调整 payload/debug 事件；`STREAM_INCLUDE_TOOL_PAYLOAD_EVENTS` 影响 SSE 与 WebSocket stream，`DEBUG_EVENTS_ENABLED` 只影响实时 stream 中的 `debug.preCall` / `debug.postCall`，这些调试事件不会写入 chat 日志。其余 `AGENT_SSE_*` / `AGENT_H2A_RENDER_*` 调优项仍只保留在本参考文档中。
...
| `DEBUG_LLM_CHAT_RECORD` | `false` | `Debug / troubleshooting` | 是否把每次 LLM chat completion 记录到 `runtime/chats/llm/<runId>_001.json` 这类三位序号文件 |
...
- LLM chat JSON trace 由 `DEBUG_LLM_CHAT_RECORD` 控制，默认关闭；开启后 `seqNo` 是当前 run 内的模型调用序号
...
- `host-tools.yml` 是 access policy、Bash 与 FileTools 的最终外部事实源
- `ai-tools.yml` 管理 `vision_recognize` 等模型能力工具配置；`image-generate`、`speech` 目前仅预留结构
...
- 配置目录固定为项目根下 `configs/`
- 旧拆分文件 `access-policy.yml`、`bash.yml`、`file-tools.yml`、`coder-prompts.yml`、`memory-prompts.yml`、`vision-recognize.yml`、`container-hub.yml`、`desktop.yml`、`cors.yml` 仍兼容读取一版，但会被新合并文件覆盖；新部署不再生成这些旧文件模板
...
```text
代码默认值 < 旧拆分 yml < 新合并 yml < 仍受支持的环境变量
```

其中：

- Host 工具新合并文件是 `configs/host-tools.yml`
