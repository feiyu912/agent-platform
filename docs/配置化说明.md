...
`file_read` / `file_glob` / `file_grep` / `file_write` / `file_edit` 是 Claude Code 风格的窄文件工具，行为配置从 `configs/host-tools.yml` 的 `file-tools` 节读取，目录权限从同一文件的 `access-policy` 节读取；参考模板是 `configs/host-tools.example.yml`。旧 `configs/file-tools.yml` 仍兼容读取，但会被 `configs/host-tools.yml` 覆盖。它们用于替代 heredoc、重定向等容易触发 bash 安全拦截的文件读写场景，不改变 bash 安全策略。
...
| YAML 字段 | 默认值 | 标签 | 说明 |
|---|---|---|---|
| `working-directory` | `.` | `Advanced / operator` | `file_read` / `file_glob` / `file_grep` / `file_write` / `file_edit` 解析相对路径时使用的工作目录；路径权限仍以 `host-tools.access-policy` 为准 |
...
| `require-read-before-write` | `true` | `Advanced / operator` | 已有文件写入或编辑前是否必须先被文件工具观察到 |
...
默认 `accessLevel=default` 下，workspace、agent、当前 agent skills 读免审，workspace 写免审；除此之外 read/glob/grep/write/edit 都按 `host-tools.access-policy` 触发审批或自动处理。旧字段 `allowed-read-paths` / `allowed-write-paths` 已移除，仍出现在 `configs/host-tools.yml` 的 `file-tools` 节或旧 `configs/file-tools.yml` 会导致启动失败。
