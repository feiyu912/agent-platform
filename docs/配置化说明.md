...
approval action 支持 `allow`、`hitl`、`auto`、`block`。`auto_approve` 默认继承 `default` 边界，越界读、复杂 bash 与不透明命令自动批准，越界写仍触发 HITL；`full_access` 默认把 host path 读写根设为 `/`，但仍保留 bashsec hard block。
