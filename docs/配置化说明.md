...
`.env.example` 现在只保留推荐给普通部署者的最终用户配置入口。本页则保留完整的高级、排障和低频配置说明；host 工具目录权限已统一迁移到 `configs/access-policy.yml`。
...
Bash 工具从 `configs/bash.yml` 读取命令白名单、超时和可选 host shell 覆盖；目录权限统一从 `configs/access-policy.yml` 读取。参考模板是 `configs/bash.example.yml`。
...
旧字段 `allowed-paths`、`path-checked-commands`、`path-check-bypass-commands` 已迁移到 `configs/access-policy.yml`；启动时仍出现这些字段会直接报错。

### Access Policy 目录权限

`configs/access-policy.yml` 是 host 工具目录权限的唯一事实源，`bash` 与 FileTools 共用。请求可传 `accessLevel`，合法值为 `default`、`auto_approve`、`full_access`，缺省为 `default`。
...
`file_read` / `file_grep` / `file_write` / `file_edit` 是 Claude Code 风格的窄文件工具，行为配置从 `configs/file-tools.yml` 读取，目录权限从 `configs/access-policy.yml` 读取；参考模板是 `configs/file-tools.example.yml`。它们用于替代 heredoc、重定向等容易触发 bash 安全拦截的文件读写场景，不改变 bash 安全策略。

`file_write` / `file_edit` 成功结果会包含 `lineStats`，字段为 `addedLines`、`deletedLines`、`editedLines`。统计基于写入前后完整文件内容的行级 diff；新建文件等价于从空内容写入新内容。

| YAML 字段 | 默认值 | 标签 | 说明 |
|---|---|---|---|
| `working-directory` | `.` | `Advanced / operator` | `file_read` / `file_grep` / `file_write` / `file_edit` 解析相对路径时使用的工作目录；路径权限仍以 `access-policy.yml` 为准 |
...
默认 `accessLevel=default` 下，workspace、agent、当前 agent skills 读免审，workspace 写免审；除此之外 read/grep/write/edit 都按 `access-policy.yml` 触发审批或自动处理。旧字段 `allowed-read-paths` / `allowed-write-paths` 已移除，仍出现在 `configs/file-tools.yml` 会导致启动失败。
...
配置从 `configs/vision-recognize.yml` 读取，参考模板是 `configs/vision-recognize.example.yml`。
...
图片来源支持当前 chat 附件 `reference_name`，以及受 `access-policy.yml` 读权限约束的本地 `file_path`。v1 不支持远程 URL。
...
| `DEBUG_LLM_CHAT_RECORD` | `false` | `Debug / troubleshooting` | 是否把每次 LLM chat completion 记录到 `runtime/chats/llm/run_<seq>.json` |
...
- LLM chat JSON trace 由 `DEBUG_LLM_CHAT_RECORD` 控制，默认关闭；开启后 `seq` 是当前 run 内的模型调用序号，平铺文件可能被后续 run 覆盖
...
- Bash 与 FileTools 的外部事实源分别是 `configs/bash.yml` 与 `configs/file-tools.yml`
